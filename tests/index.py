...
from mixology.contracts import SpecificationProvider
...
class Index(SpecificationProvider):
                ...
                packages_by_name[name].sort(
                    key=cmp_to_key(
                        lambda x, y:
                            ...
                            else -1 * int(x.version < y.version or -1)
                    )
                )
    ...
    def is_requirement_satisfied_by(self, requirement, activated, package):
        ...
        return requirement.constraint.match(package.version)
    ...
    def search_for(self, dependency):
        ...
        for spec in self._packages[dependency.name]:
            ...
            if dependency.constraint.match(spec.version):
                results.append(spec)
